...
db = client.CovidModel

beds1 = db.Beds
beds = pd.DataFrame(list(beds1.find()))

agegroups1 = db.Agegroups
agegroups = pd.DataFrame(list(agegroups1.find()))

probabilities1 = db.Probabilities
probabilities = pd.DataFrame(list(probabilities1.find()))



covid_data1 = db.Deaths_global_narrow
covid_data = pd.DataFrame(list(covid_data1.find()))
...
covid_data_plot = covid_data.groupby(['Date']).sum()["Value"].reset_index()
covid_data.plot(x = ('Date'), y = ("Value"), figsize=(20, 10), title="Covid-19 total fatalities (world)")
...
{'R_0_end': 0.6219758003697701,
 'R_0_start': 3.5184772112183333,
 'k': 4.782797860029632,
 'prob_C_to_D': 0.5654723056888374,
 'prob_I_to_C': 0.07105726268170251,
 's': 0.002373867155360338,
 'x0': 84.22435137072891}
